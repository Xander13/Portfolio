...
class GenerativeSnake {
    ...
    generatePath() {
        ...
        for (let my = 0; my < mazeRows; my++) {
            for (let mx = 0; mx < mazeCols - 1; mx++) {
                // Wall between cell (mx, my) and cell (mx+1, my)
                // This is wallsV[my * (mazeCols + 1) + (mx + 1)]
                if (wallsV[my * (mazeCols + 1) + (mx + 1)] === 0) { // No wall -> Connected
                    // Merge Cell(mx, my) with Cell(mx+1, my)
                    // Left cell (mx, my) right edge: TR-BR
                    // Right cell (mx+1, my) left edge: TL-BL
                    ...
                    removeAdj(left_tr, left_br); // Remove right edge of left cell
                    removeAdj(right_tl, right_bl); // Remove left edge of right cell

                    setAdj(left_tr, right_tl); // Connect top-top
                    setAdj(left_br, right_bl); // Connect bottom-bottom
                }
            }
        }
    ...
    drawEndCap(x, y, dx, dy, img) {
        if (!img || !img.complete) return;
        this.ctx.save();
        this.ctx.translate(x, y);

        // Calculate rotation based on direction + 90 deg tweak
        // Restoring + Math.PI / 2 because user said "rotation isn't correct" after I removed it.
        // If image is Native Up. +90 makes it Right.
        // If direction is Right (0). Angle is 90 (Down).
        // If direction is Up (-90). Angle is 0 (Right).
        // Let's assume standard "End Cap" behavior: Cap is perpendicular to line.
        // Angle of line + 90 puts the cap "across" the end.
        const angle = Math.atan2(dy, dx) + Math.PI / 2;
        this.ctx.rotate(angle);

        // Scale cap 
        const widthBoost = 2; // User requested +2px
        const w = this.thickness + widthBoost;
        const scale = w / img.naturalWidth;
        const h = img.naturalHeight * scale;

        // Offset for a 2px gap between the line end and the wrist/ankle
        const gap = 2;

        // Draw centered on X (-w/2) and above/below Y?
        // If we rotated 90. X is across width. Y is along length.
        // We want image to start at Y=0 (or gap) and go outwards (negative Y? or positive?)
        // Standard canvas rotation: X axis rotates.
        // If we want to draw "At the end".
        // Let's go back to standard:
        // drawX = -w / 2 (Center horizontally relative to thickness).
        // drawY = -h - gap (Draw "behind" the cursor? No).

        // If we are at the End Point, and we faced the direction.
        // We want the cap to be attached to the end.
        // If we rotate +90.
        // Vector (1, 0) -> Angle 90.
        // X axis points Down. Y axis points Right? No.
        // Rotate 90: X->Y, Y->-X.
        // So +X is Down. +Y is Left?
        // This is confusing.

        // Let's rely on standard practice:
        // Translate to x,y. Rotate by atan2(dy, dx) + PI/2.
        // Draw Image at (-w/2, -h).
        // If Image is Up-Oriented Hand. 
        // Line goes Up. Angle = -90. Rotated = 0.
        // Image drawn upright at (-w/2, -h). 
        // Canvas (0,0) is at Line End.
        // Image (-w/2, -h) is ABOVE the line end.
        // So it sits on top of the line end. Perfect.
    ...
    draw() {
        ...
        if (this.endCapType !== 'none' && endImg) {
            ...
            if (Math.abs(headDx) > 0.01 || Math.abs(headDy) > 0.01) {
                this.drawEndCap(headX, headY, headDx, headDy, endImg);
            }
        }
        ...
        if (this.obstacleType !== 'none' && this.obstacleCell) {
            ...
            if (img && img.complete) {
                ...
                this.ctx.drawImage(img, pixelX - renderW / 2, pixelY - renderH / 2, renderW, renderH);
            }
        }
    }
}
...
function initMaze() {
    const canvas = document.getElementById('fancyFooter');
    if (!canvas) return;
    ...
    function resize() {
        ...
        const height = 800; // Fixed height per user request
        ...
        canvas.height = height * dpr; // Will be overwritten by grid snap logic? NO, fixed 800px.

        ctx.resetTransform();
        // ctx.scale(dpr, dpr); // Will set transform later

        // Calculate optimal thickness to fit width/height perfectly
        // We want cols and rows to be even numbers for the maze algorithm
        // GridSize = Thickness + Gap
        // Width = Cols * GridSize
        // Height = Rows * GridSize

        // Define desired canvas height or max height?
        // User said: "I don't need a 100vh height... Just shrink the canvas height to help close the snake 1 and 2 gaps"
        // This implies we should let the content dictate the height, but fill the width.
        // We start with window.innerHeight as a target/max but snap to grid.
        ...
        // Adjustable height to match grid exactly?
        // User said "lets just do 800px".
        // If 800 is not divisible by gridSize, we have a gap or clip.
        // We should adjust canvas height to match logic?
        // Or scale height.
        // Let's use the SCALING logic again to force fit 800px.

        // Actual canvas height is fixed 800.
        // Grid covers: rows * gridSize.
        const scaleY = height / (rows * gridSize);

        // Apply Scale
        ctx.setTransform(dpr, 0, 0, dpr * scaleY, 0, 0); // Stretch vertically to fill 800px exactly

        // Layout Decision:
        // Force Horizontal on Mobile (<768)
        // Otherwise use random decision
        const isVertical = (width > 768) && randomSplitDecision;

        if (isVertical) {
            // 50/50 Vertical Split
            const midCol = cols / 2; // cols is even, so this is integer
            ...
            const midRow = Math.floor(rows / 2); // Split exactly in half
